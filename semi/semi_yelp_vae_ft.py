...
def vae(x, y, is_train, opt, epoch_t, opt_t=None):
    ...
    x_lat = z if opt.use_z else mu
    logits = discriminator_linear(x_lat, opt, prefix='classify_', is_train=is_train)  # batch * 1
...
def run_model(opt, train, val, test, train_lab, val_lab, test_lab, wordtoix, ixtoword):
    ...
    with tf.device('/gpu:0'):
        ...
        loss_, train_op_, accuracy_ = vae(x_, y_, is_train_, opt, epoch_t)

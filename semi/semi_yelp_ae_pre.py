...
def run_model(opt, train, val, test, test_lab, wordtoix, ixtoword):
    ...
    with tf.device('/gpu:0'):
        ...
        res_, loss_, train_op_ = ae(x_, x_org_, is_train_, opt, epoch_t)
    ...
    def run_epoch(sess, epoch, mode, print_freq=-1, display_sent=-1, train_writer=None):
        ...
        for _, index in kf:
            ...
            if print_freq>0 and local_t%print_freq==0:
                print("%s Iter %d: loss %.4f, time %.1fs" % 
                    (mode, local_t, acc_loss/acc_n, time.time()-start_time))
                sys.stdout.flush()
        ...
        if display_sent>0:
            ...
            res = sess.run(res_, feed_dict={x_: x_d, x_org_: x_d_org, is_train_:is_train})
    ...
    with tf.Session(config = config) as sess:
        ...
        for epoch in range(opt.max_epochs):
            ...
            _ = run_epoch(sess, epoch, 'train', opt.print_freq, display_sent=1)
            val_loss = run_epoch(sess, epoch, 'val', display_sent=1)
            test_loss = run_epoch(sess, epoch, 'test', display_sent=1)

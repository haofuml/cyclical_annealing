...
class Options(object):
    def __init__(self):
        ...
        self.save_path = "./save/vae_pre"
        self.log_path = "./log/vae_pre"
        self.print_freq = 100
        self.save_freq_ep = -1
        self.save_last = True
...
def vae(beta, x, x_org, is_train, opt, epoch_t, opt_t=None):
...
def run_model(opt, train, val, test, test_lab, wordtoix, ixtoword):
    ...
    with tf.device('/gpu:0'):
        ...
        res_, loss_, rec_loss_, kl_loss_, train_op_ = vae(beta_, x_, x_org_, is_train_, opt, epoch_t)
    ...
    def run_epoch(sess, epoch, mode, print_freq=-1, display_sent=-1, train_writer=None):
        ...
        for _, index in kf:
            ...
            if mode == 'train':
                if opt.vae_anneal:
                    beta_t = opt.max_beta * np.minimum(float(global_t)/(epoch_t*opt.anneal_ep), 1.)
                else:
                    beta_t = opt.max_beta
            else:
                beta_t = opt.max_beta
            ...
            feed_t = {beta_: beta_t, x_: x_b, x_org_: x_b_org, is_train_:is_train}
            ...
            if print_freq>0 and local_t%print_freq==0:
                print("%s Iter %d: loss %.4f, rec %.4f, kl %.4f, beta %.4f, time %.1fs" % 
                    (mode, local_t, acc_loss/acc_n, acc_rec/acc_n, acc_kl/acc_n, beta_t, time.time()-start_time))
                sys.stdout.flush()

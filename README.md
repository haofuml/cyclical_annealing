...
Comparison between (a) traditional monotonic and (b) proposed cyclical annealing schedules. The training objective of **V ariational Autoencoder (VAE)** is an ELBO of log p(x) (ie, β=1). However, when trained with a constanty schedule β=1, the KL term L_R vanishes to 0, leading to the **KL vanishing** issue.   

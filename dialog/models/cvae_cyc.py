...
class BaseTFModel(object):
    ...
    def print_model_stats(tvars):
        ...
        for variable in tvars:
            ...
            logger.info("Trainable %s with %d parameters" % (variable.name, variable_parametes))
            total_parameters += variable_parametes
        logger.info("Total number of trainable parameters is %d" % total_parameters)
    ...
    def print_loss(prefix, loss_names, losses, postfix):
        ...
        logger.info(template % tuple(values))
        return avg_losses
    ...
    def optimize(self, sess, config, loss, log_dir):
        ...
        if config.op == "adam":
            ...
            optimizer = tf.train.AdamOptimizer(self.learning_rate_cyc)
        ...
        logger.info("Save summary to %s" % log_dir)
...
class KgRnnCVAE(BaseTFModel):
    ...
    def __init__(self, sess, config, api, log_dir, forward, scope=None):
        ...
        with tf.name_scope("io"):
            ...
            self.global_t_cyc = (self.global_t - 1) % cycle
            tmp = tf.to_float(self.global_t_cyc)/ cycle
            self.learning_rate_cyc = 0.5*config.init_lr*(1+tf.cos(tmp*np.pi))
        ...
        with variable_scope.variable_scope("generationNetwork"):
            ...
            if (self.beam_width != 0) and forward:
                dec_init_state = tf.contrib.seq2seq.tile_batch(dec_init_state, multiplier=self.beam_width)
        ...
        with variable_scope.variable_scope("decoder"):
            ...
            if (self.beam_width != 0) and forward:
                BS_decoder = tf.contrib.seq2seq.BeamSearchDecoder(
                    cell=dec_cell,
                    embedding=embedding,
                    start_tokens=tf.fill([batch_size], self.go_id),
                ...
                # Dynamic decoding
                with variable_scope.variable_scope("rnn") as bs_scope:
                    outputs, final_states, _ = tf.contrib.seq2seq.dynamic_decode(
                        BS_decoder, maximum_iterations=self.max_utt_len, scope=bs_scope, impute_finished=False)
                self.dec_out_words  = outputs.predicted_ids[:,:,0]
                self.dec_out_lens = final_states.lengths[:,0]
        ...
        if not forward:
            with variable_scope.variable_scope("loss"):
                ...
                if log_dir is not None:
                    kl_weights = tf.minimum(tf.to_float(self.global_t_cyc+1)/config.full_kl_step, 1.0)
                else:
                    kl_weights = tf.constant(1.0)

                self.kl_w = kl_weights
                self.elbo = self.avg_rc_loss + kl_weights * self.avg_kld
                aug_elbo = self.avg_bow_loss + self.avg_da_loss + self.elbo if config.use_bow else self.elbo
    ...
    def test_mul_ref(self, sess, test_feed, num_batch=None, repeat=5, dest=sys.stdout):

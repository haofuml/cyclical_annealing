...
tf.app.flags.DEFINE_string("sub_dir", "cvae_bl_elbo", "the dir to load checkpoint for forward only")
    ...
    with tf.Session(config=sess_config) as sess:
        ...
        if not FLAGS.forward_only:
            ...
            for epoch in range(config.max_epoch):
                logger.info(">> Epoch %d with lr %f" % (epoch, model.learning_rate.eval()))
